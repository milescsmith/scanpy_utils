...
def velocity_compare(
    ...
    for _ in feature_list:
        ...
        for index, item in enumerate(adatas):
            ...
            xnew = np.linspace(
                0,
                np.percentile(make_dense(adatas[item][:, basis].layers[xkey]), 98),
            )
...
def plot_grouped_velo_grid(adata: AnnData,
                           group: str,
    ...
    quiver_dict = {item: {k: v for k, v in zip(["XY_grid", "V_grid"], compute_velocity_on_grid(
        X_emb=np.array(obj_dict[item].obsm[f"X_{basis}"]
                    [:, get_components(components, basis)]),
        V_emb=np.array(obj_dict[item].obsm[f"{vkey}_{basis}"]
                    [:, get_components(components, basis)]),
        autoscale=True,))}
    ...
    for index, item in enumerate(obj_dict):
        embed_points = pd.DataFrame(obj_dict[item].obsm[f"X_{basis}"])\
            .rename(columns={0: f"{basis}_1",
                            1: f"{basis}_2"})
        exprs = interpret_colorkey(obj_dict[item], color, layer, 0.95)

        axs[index].scatter(x=embed_points[f'{basis}_1'],
...
def plot_dictionary_velo_grid(adata_dict: Dict[str, AnnData],
                              basis: Optional[str] = None,
                              color: str = "YES1",
                              layer: str = "velocity",
                              vkey: str = "velocity",
    ...
    for index, item in enumerate(adata_dict):
        ...
        quiver_data = {k: v for k, v in zip(["XY_grid", "V_grid"], compute_velocity_on_grid(
            X_emb=np.array(adata_dict[item].obsm[f"X_{basis}"]
                        [:, get_components(components, basis)]),
            V_emb=np.array(adata_dict[item].obsm[f"{vkey}_{basis}"]
                        [:, get_components(components, basis)]),
            autoscale=True,))}

        size = 4 * default_size(adata_dict[item]) if size is None else size

        embed_points = pd.DataFrame(adata_dict[item].obsm[f"X_{basis}"])\
            .rename(columns={0: f"{basis}_1",
                            1: f"{basis}_2"})
        exprs = interpret_colorkey(adata_dict[item], color, layer, 0.95)

        ax = fig.add_subplot(1, len(adata_dict), index+1)
        ax.scatter(x=embed_points[f'{basis}_1'],
                        y=embed_points[f'{basis}_2'],
                        c=exprs,
                        s=1,
                        cmap=colormap[item])
